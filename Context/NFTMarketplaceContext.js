...
const projectId = "2F0fIm5QDMFmCFWj1RQJarIaQrx";
const projectSecretKey = "6e5cd82f454db8033cd1fa546721cd5c";
const auth = `Basic ${Buffer.from(`${projectId}:${projectSecretKey}`).toString(
  "base64"
)}`;

const subdomain = "https://flipindotnart.infura-ipfs.io";
...
export const NFTMarketplaceProvider = ({ children }) => {
  const titleData = "Collect, Sell, Buy, Barter & Trade any NFT Assets";
  ...
  const checkIfWalletConnected = async () => {
    try {
      ...
      setError("Something wrong while connecting to the wallet");
      console.log(error);
      setOpenError(true);
    }
  };
    ...
    try {
      if (currentAccount) {
        const provider = new ethers.providers.JsonRpcProvider(
          "https://polygon-mumbai.g.alchemy.com/v2/-7VjZKjfJCuaGOgyX3CQtQOsKuBtODTz",  
        );
